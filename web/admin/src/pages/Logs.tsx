...
import { Table, Select, DatePicker, Button, Tabs, message } from 'antd';
...
export default function Logs() {
  ...
  useEffect(() => {
    if(selProject){setBuildings(allBuildings.filter((b:any)=>Number(b.project_id)===Number(selProject))); if (!restoring.current) {setSelBuilding(null);setSelDevice(null);}}
    else{setBuildings([]);setSelBuilding(null);setSelDevice(null);}
  }, [selProject]);
  useEffect(() => { if(selBuilding){api.get('/devices?building_id='+selBuilding).then(r=>{setDevices(r.data); if (!restoring.current) setSelDevice(null);});}else{setDevices([]);setSelDevice(null);} }, [selBuilding]);
  ...
  const getDeviceName = () => devices.find((d:any)=>Number(d.id)===selDevice)?.name||'-';
  const getBldName = () => allBuildings.find((b:any)=>Number(b.id)===selBuilding)?.name||'-';
  const getProjName = () => projects.find((p:any)=>Number(p.id)===selProject)?.name||'-';
  ...
  const exportCSV = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = (activeTab==='controls'?'controls':'telemetry')+'_'+dayjs().format('YYYYMMDD')+'.csv';
      a.click();
  ...
  const controlCols = [
    ...
    { title:'楼宇', dataIndex:'building_name', width:100 },
